...
- **D-03:** Detection compares Stripe-side paid events against the transfer ledger. **RESOLVED (2026-06-20):** the **Stripe API is the source of truth** — the sweep lists recent paid Checkout Sessions from Stripe (server-only key) and anti-joins against `wp_transfers` paid. This is the only design that catches a *never-delivered* webhook (which leaves NO `webhook_events` row), so it satisfies the pilot DoD ("reconciliation catches a deliberately-dropped webhook"). `webhook_events` (migration 0003) + `wp_transfers` remain available for a secondary effect-failure cross-check, but the Stripe API is primary. Still detect + alert ONLY (D-01) — never writes `paid`. The exact query/lookback window (e.g. only flag discrepancies older than ~10 min to avoid in-flight false positives) is planner territory.
